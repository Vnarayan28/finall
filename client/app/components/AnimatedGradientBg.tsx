...
export default function AnimatedGradientBg({ isDarkMode }: AnimatedGradientBgProps) {
  return (
    <div className={`fixed inset-0 -z-10 h-full w-full 
      ${isDarkMode ? 'bg-gray-950' : 'bg-slate-100'} // Slightly lighter light mode bg
      bg-[radial-gradient(#e5e7eb_0.5px,transparent_0.5px)] 
      dark:bg-[radial-gradient(#ffffff10_0.5px,transparent_0.5px)] // More subtle dots in dark mode
      [background-size:16px_16px]`}>
      <div className={`absolute inset-0 -z-20 animate-aurora 
        ${isDarkMode ? 'bg-gradient-to-r from-purple-900/50 via-gray-950 to-blue-900/50' // Adjusted aurora
