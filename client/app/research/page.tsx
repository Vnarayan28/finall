...
const LoadingState = ({ isDarkMode, topic }: { isDarkMode: boolean; topic: string }) => (
  <motion.div
    className={`min-h-screen flex flex-col items-center justify-center relative ${isDarkMode ? "bg-gray-900" : "bg-gray-50"
      }`}
    initial={{ opacity: 0 }}
    animate={{ opacity: 1 }}
  >
    ...
    <motion.h1
      className={`text-4xl font-bold bg-clip-text text-transparent ${isDarkMode
...
const ErrorState = ({ isDarkMode, error, onRetry }: { isDarkMode: boolean; error: string; onRetry: () => void }) => (
  ...
  >
    ...
    <motion.div
      initial={{ scale: 0 }}
      animate={{ scale: 1 }}
      className={`p-6 rounded-full mb-6 z-10 ${isDarkMode ? "bg-red-500/20" : "bg-red-400/20"
        }`}
    >
      <AlertCircle className={`h-16 w-16 ${isDarkMode ? "text-red-400" : "text-red-500"
        }`} />
    </motion.div>

    <h1 className={`text-4xl font-bold mb-4 z-10 ${isDarkMode ? "text-red-300" : "text-red-500"
      }`}>
      Oops!
    </h1>

    <p className={`mt-2 max-w-lg text-center text-lg z-10 ${isDarkMode ? "text-gray-200/90" : "text-gray-700/90"
      }`}>
      {error}
    </p>
    ...
    <motion.button
      ...
      whileTap={{ scale: 0.95 }}
      className={`mt-8 px-8 py-3.5 rounded-xl font-semibold text-lg transition-all duration-200 shadow-lg z-10 ${isDarkMode
    ...
    >
      ...
      <div className="absolute inset-0 pointer-events-none z-0">
        {[...Array(30)].map((_, i) => (
          <motion.div
            key={i}
            className={`absolute w-1.5 h-1.5 rounded-full blur-[1px] ${isDarkMode
      ...
      <motion.button
        onClick={() => push('/')}
        className={`absolute top-6 left-6 p-2 rounded-full z-50 ${isDarkMode ? 'text-purple-400 hover:bg-purple-900/50' : 'text-purple-600 hover:bg-purple-100'
          }`}
      ...
      <motion.button
        onClick={toggleTheme}
        className={`absolute top-6 right-6 p-3 rounded-full z-50 ${isDarkMode
      ...
      <div className="container mx-auto p-6 md:p-12 lg:p-16 relative z-10">
        ...
        >
          <motion.div
            className={`inline-flex p-4 rounded-2xl mb-4 ${isDarkMode
          ...
          >
            <Sparkles className={`w-8 h-8 ${isDarkMode ? 'text-white' : 'text-gray-100'
              }`} />
          </motion.div>

          <h1 className={`text-5xl font-extrabold bg-clip-text text-transparent ${isDarkMode
              ? "bg-gradient-to-r from-purple-200 to-blue-200"
              : "bg-gradient-to-r from-purple-600 to-blue-600"
            }`}>
            Explore {topic}
          ...
          <p className={`text-xl max-w-2xl mx-auto ${isDarkMode ? "text-gray-300/90" : "text-gray-600/90"
            }`}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
          {videos.map((video, index) => (
            <motion.div
              ...
              className={`relative group cursor-pointer rounded-2xl overflow-hidden transition-transform ${isDarkMode
            ...
            >
              <div className="relative aspect-video">
                ...
                <div className="absolute bottom-0 left-0 right-0 p-4">
                  <h3 className={`font-bold text-lg line-clamp-2 ${isDarkMode ? "text-white" : "text-white"
                    } drop-shadow-lg`}>
                    {video.title}
                  </h3>
                  <p className={`text-sm mt-1.5 font-medium ${isDarkMode ? "text-gray-300" : "text-gray-200"
                    }`}>
                    {video.channel}
                  </p>
                </div>
                <div className={`absolute top-3 right-3 px-2.5 py-1 rounded-full text-xs font-medium ${isDarkMode
              ...
              <div className={`p-4 border-t ${isDarkMode
                ...
                <p className={`text-sm line-clamp-3 leading-relaxed ${isDarkMode ? "text-gray-200/90" : "text-gray-700/90"
                  }`}>
                  {video.description}
                </p>
              </div>
          ...
          {selectedVideo && (
            ...
            >
              <motion.button
                ...
                whileHover={{ scale: 1.05 }}
                whileTap={{ scale: 0.95 }}
                className={`inline-flex items-center px-8 py-4 rounded-xl text-lg font-semibold shadow-lg transition-all ${isDarkMode
              ...
              >
                <Sparkles className={`mr-3 h-6 w-6 ${isDarkMode ? 'text-white' : 'text-gray-900'
                  } animate-pulse`} />
